...
| | |
|---|---|
| timeout    | **86 384 s (≈ 24 h)** wall-clock per run — deliberately huge so it is *not* the binding limit: with only memory enforced, an unfinished run is a genuine **memout**, not a timeout. copperbench derives runsolver's `-W` (`timeout + 10 s`) and SLURM's `--time` (`timeout + 15 s`) from it, and it renders `--time` from a Python `timedelta` — which formats anything **≥ 1 day** as `"1 day, 0:00:15"`, a string `sbatch` rejects (`Invalid directive … day,`). So **86 384 is the ceiling**: it gives `--time=23:59:59`, the largest value that stays under a day. It must also be **≤ the sunnycove partition's MaxTime** (`sinfo -p sunnycove -o %l`); lower it if the partition rejects ~24 h jobs. A run that somehow hits even this cap is still reported `Memout` — the cap sits comfortably above any real memout timescale (minutes) |
